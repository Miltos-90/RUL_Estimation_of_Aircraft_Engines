...
Following the competition, the datasets were posted at the data repository of the [NASA Ames Prognostics Center of Excellence](https://ti.arc.nasa.gov/tech/dash/groups/pcoe/prognostic-data-repository/#turbofan). Note that this repository hosts a number of interesting datasets that can be used in a prognostic context
...
```{r append_health, warning=FALSE, message=FALSE}
train %>%
  group_by(unit_no) %>%
...
```{r append_health2, warning=FALSE, message=FALSE}
ttf <- train %>%
  group_by(unit_no) %>%
...
```{r sensor_timeseries, warning=FALSE, message=FALSE, fig.width=10, fig.height=14}
...
```{r sensor_data_unique_vals, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
train %>%
...
```{r sensor_data_unique_vals2, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
...
```{r sensor_data_4, warning=FALSE, message=FALSE, fig.width=7, fig.height=7}
...
```{r plot_4, warning=FALSE, message=FALSE}
...
```{r cluster_ops, warning=FALSE, message=FALSE}
set.seed(1)
...
```{r cluster_ops2, warning=FALSE, message=FALSE}
# Append cluster no. to the training set
train$op_cluster <- factor(op_clusters$cluster)
```

This is potentially the most useful information we have extracted from the data so far. By significantly reducing the number of operating conditions into a manageable size, we have the ability to completely remove their effects from the sesnsor measurements.
...
```{r sensor_data_5, warning=FALSE, message=FALSE, fig.height=14, fig.width=10}
# Generate a plot list
plotlist <- lapply(sensor.cols, 
                   function(col){ 
                     train %>%
...
```{r sensor_data_51, warning=FALSE, message=FALSE, fig.height=14, fig.width=10}
...
```{r sensor_timeseries_stdized_2, warning=FALSE, message=FALSE}
...
```{r violin_plot, warning=FALSE, message=FALSE}
...
```{r violin_plot_2, warning=FALSE, message=FALSE}
train %>%
...
```{r violin_plot_3, warning=FALSE, message=FALSE}
# Append the standardized sensors
...
```{r sensor_data_corrplot, fig.width=7, fig.height=7}
...
Alright, there are several sensor that are correlated with each other. We need some form of dimensionality reduction. 
...
```{r ica3, fig.width = 13, fig.height = 5, message=FALSE, warning=FALSE}
train %>%
  select(ttf, unit_no) %>%
...
```{r splitter, warning=FALSE, message=FALSE}
...
```{r wrappers, warning=FALSE, message=FALSE}
...
```{r wrappers_2, warning=FALSE, message=FALSE}
...
```{r wrappers_3, warning=FALSE, message=FALSE}
...
Now we have our processed datasets. We will generate a set of trained models on the data usig caretEnsemble. The models to be generated are the following:
...
We can see that all models have similar-ish average performance, but some of them are quite unstable. Furthermore, they present fairly uncorrelated results, which makes them good candidates for an ensemble. Let's do a simple, linear greedy optimization:

```{r greedy_ensemble, warning=FALSE, message=FALSE}
# Reproductibility
set.seed(1)
...
The weighted average ensemble did not manage to outperform all base learners, with a cross-validation error metric of around 248. Also, the residual vs fitted plot clearly shows that the model is biased. It tends to over- or under-predict, depending on the acrual lifetime. Let's try a Generalized Boosted Regression Model as a meta-learner. Note that these sophisticated ensembles require a lot of data to be trained properly, and they require large sets of resamples to train on. Nevertheless, we'll give it a shot:

```{r gbm_ensemble, warning=FALSE, message=FALSE}
# Reproducibility
set.seed(1)
...
The results have not improved at all compared to the weighter averaging approach we used earlier. The minimum observed error metric is around 252. Let's give XGBoost a shot, and tune it manually - Caret tends to be slow.

```{r xgb_ensemble, warning=FALSE, message=FALSE, fig.width=9}
# Reproducibility
set.seed(1)
...
The end-result surpasses all previous models tried so far. The error metric is significantly reduced. However, this could be the result of massive overfitting.

Let's see how our approaches fare on the validation set. Note that we will also include some base learners in our comparison:

```{r val_set, warning=FALSE}
set.seed(1)
...
Indeed, the error metric is higher on the validation set - an overfitiing symptom. Let's apply our best approach on the prediction set, and generate the submission file:

```{r test_set, warning=FALSE, results="hide"}
...
# Conclusions
We did get a score of 986.342, which gives us the 7th place on the test set. Too bad we're late for the actual competition! Our solution ended up being rather rich mixture of supervised and unsupervised learning, as presented below.
